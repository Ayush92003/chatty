...
const ChatContainer = () => {
  ...
  // ✅ WhatsApp-like Status Icon Function
  const renderStatusIcon = (status) => {
    if (!status || status === "sent")
      return <FaCheck size={11} className="text-gray-400" />;
    if (status === "delivered")
      return <FaCheckDouble size={11} className="text-gray-400" />;
    if (status === "seen")
      return <FaCheckDouble size={11} className="text-blue-500" />;
    return null;
  };
  ...
  const markUnseenMessagesAsSeen = async () => {
    try {
      ...
      const unseen = messages.filter(
        (m) =>
          m.receiverId === authUser._id && (!m.status || m.status !== "seen")
      );
      for (const msg of unseen) {
        await axiosInstance.put(`/messages/${msg._id}/mark-seen`);
        useChatStore.setState((state) => ({
          messages: state.messages.map((x) =>
            x._id === msg._id ? { ...x, status: "seen" } : x
          ),
        }));
      }
    } catch (err) {
      console.error("markUnseenMessagesAsSeen error:", err?.message);
    }
  };
